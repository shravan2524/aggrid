...
import {
  ItemType, readAll as readAllFn, create as createFn, update as updateFn, patch as patchFn,
} from 'services/users';
import ProgressBar from 'app/utils/ProgressBar';
import { toast } from 'react-hot-toast';

const moduleName = 'users';
const moduleTitle = 'User';
...
export const create = createAsyncThunk(`create${moduleName}`, async (payload: ItemType) => createFn(payload));
export const update = createAsyncThunk(`update${moduleName}`, async (payload: ItemType) => updateFn(payload));
export const patch = createAsyncThunk(`update${moduleName}`, async (payload: ItemType) => patchFn(payload));
...
export const readAllSelector = createSelector(
  Selector,
  (state: State): ItemType[] => state.rows,
);
